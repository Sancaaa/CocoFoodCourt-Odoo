...
Admin dapat memantau tingkat kepadatan atau okupansi meja berdasarkan jam dan hari dalam seminggu melalui grafik batang bertumpuk (\textit{stacked bar chart}). Hal ini membantu manajemen dalam menganalisis waktu sibuk (\textit{peak hours}) di \textit{food court}.
\begin{figure}[htbp]
\centering
...
\label{fig:analytics_occupancy}
\end{figure}

\textbf{Analitik Pendapatan Tenant:}
Tampilan ini menyajikan grafik batang yang membandingkan total pendapatan (termasuk pajak) dari masing-masing tenant yang beroperasi. Data ini memudahkan admin untuk mengevaluasi tenant dengan performa penjualan tertinggi.
\begin{figure}[htbp]
\centering
...
\label{fig:analytics_revenue}
\end{figure}

\textbf{Kontribusi Penjualan Tenant:}
Grafik ini menunjukkan seberapa besar kontribusi jumlah pesanan (\textit{paid/done orders}) dari setiap tenant terhadap total keseluruhan transaksi di \textit{food court}. Admin dapat menggunakan laporan ini sebagai landasan untuk perhitungan bagi hasil atau evaluasi kontrak.
\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\linewidth]{res/analytics_tenant sales contribution.png}
\caption{Tampilan Kontribusi Penjualan Tenant di Backend Odoo}
\label{fig:analytics_contribution}
\end{figure}

\textbf{Tren Reservasi:}
Admin dapat memantau tren jumlah reservasi aktif dari hari ke hari melalui grafik area (\textit{area chart}). Tampilan ini sangat berguna untuk melihat pola dan fluktuasi minat pengunjung \textit{food court} selama periode waktu tertentu.
\begin{figure}[htbp]
\centering
